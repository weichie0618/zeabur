...
import { SalespersonAuthProvider, useSalespersonAuth } from './contexts/SalespersonAuthContext';
...
const LoadingScreen: React.FC = () => (
  <div className="flex items-center justify-center min-h-screen bg-gray-100">
    <div className="text-center">
      ...
      <p className="text-gray-600">正在載入業務平台...</p>
    </div>
  </div>
);
...
const ErrorScreen: React.FC<{ error: string; onRetry: () => void }> = ({ error, onRetry }) => (
  <div className="flex items-center justify-center min-h-screen bg-gray-100">
    <div className="text-center max-w-md p-6">
      ...
      <h2 className="text-xl font-semibold text-gray-900 mb-2">認證失敗</h2>
...
// 主要佈局內容
const SalesLayoutContent: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const { isAuthenticated, isLoading, error, salespersonInfo, authenticate, logout, clearError } = useSalespersonAuth();
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-100">
        <div className="text-center max-w-md p-6">
          ...
          <h2 className="text-xl font-semibold text-gray-900 mb-2">歡迎使用業務平台</h2>
          <p className="text-gray-600 mb-4">請先完成身份驗證以存取業務功能</p>
          <button
            onClick={handleRetry}
            className="px-4 py-2 bg-amber-600 text-white rounded-lg hover:bg-amber-700 transition-colors"
          >
    ...
    <div className="flex h-screen bg-gray-100">
      ...
      <aside className={`fixed inset-y-0 left-0 z-10 w-64 bg-gray-800 text-white transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 transition-transform duration-300 ease-in-out md:static md:block`}>
        ...
        <nav className="mt-4 sm:mt-6">
          ...
          <div className="px-4 py-2 mt-4 sm:mt-6 text-xs text-gray-400 uppercase">佣金與報表</div>
          
          <SidebarLink href="/sales/commissions" active={pathname.startsWith('/sales/commissions')} onClick={closeSidebar}>
            <span className="flex items-center">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <SidebarLink href="/sales/performance" active={pathname === '/sales/performance'} onClick={closeSidebar}>
            <span className="flex items-center">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <header className="bg-white shadow-sm">
          <div className="flex justify-between items-center px-4 sm:px-6 py-3">
            ...
            <div className="flex items-center">
              {/* 使用者資訊與登出按鈕 */}
              <div className="flex items-center">
                <span className="text-gray-700 mr-2 sm:mr-4 hidden sm:inline">
                  {salespersonInfo?.storeValue || '業務人員'}
                </span>
                <button 
                  onClick={logout}
